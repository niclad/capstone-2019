...
class NNClassifier(object):
    ...
    def __init__(self):
        for i in range(10):
            with open('./DataFiles/vals%d.dat' % i, 'ab') as f: pass
        self.read_data()

    def store_data(self, cls, vals):
        with open('./DataFiles/vals%d.dat' % cls, 'ab') as f:
    ...
    def read_data(self):
        ...
        Y = []
        for i in range(10):
            X.append(np.fromfile('./DataFiles/vals%d.dat' % i, dtype=np.uint16).reshape((-1, 8)))
...
if __name__ == '__main__':
    print("RUN RAN HERE WHERE IS THIS RUN FUNCTION TAKING ME AHHHH")
    import subprocess
